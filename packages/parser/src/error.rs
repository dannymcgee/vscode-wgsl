...
impl fmt::Display for Rule {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		...
		let text = match self {
			...
			IMPORT => "`import`",
			BLOCK => "`block`",
			plus_export_modifier => "`(WGSL+) <export>`",
			SIGN => "`+` or `-`",
			...
			plus_namespaced_type => "(WGSL+) namespaced type",
			namespace => "(WGSL+) namespace",
			...
			param => "parameter",
			directive => "directive",
			enable_directive => "enable directive",
			plus_import_directive => "(WGSL+) import directive",
			program => "program",
		};
